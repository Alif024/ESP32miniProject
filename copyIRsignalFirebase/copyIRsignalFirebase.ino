...
IRRawDataType DecodedRawDataArrayToFB[RAW_DATA_ARRAY_SIZE] = { 0x7B34ED12 };
DistanceWidthTimingInfoStruct sDistanceWidthTimingInfo = { 9000, 4500, 560, 1690, 560, 560 };
DistanceWidthTimingInfoStruct sTimingInfoToFB = { 9000, 4500, 560, 1690, 560, 560 };
uint8_t sNumberOfBits = 32;
uint8_t NumberOfBitsToFB = 32;

bool sSendButtonWasActive;



void updateDistanceWidthTimingInfo(int Timing[]) {
  sDistanceWidthTimingInfo.HeaderMarkMicros = Timing[0];
  sDistanceWidthTimingInfo.HeaderSpaceMicros = Timing[1];
  sDistanceWidthTimingInfo.OneMarkMicros = Timing[2];
  sDistanceWidthTimingInfo.OneSpaceMicros = Timing[3];
  sDistanceWidthTimingInfo.ZeroMarkMicros = Timing[4];
  sDistanceWidthTimingInfo.ZeroSpaceMicros = Timing[5];
}
...
void sendDistanceWidthTimingInfoFB(String path) {
  ...
  sendDistanceWidthTiming.add(sTimingInfoToFB.HeaderMarkMicros);
  sendDistanceWidthTiming.add(sTimingInfoToFB.HeaderSpaceMicros);
  sendDistanceWidthTiming.add(sTimingInfoToFB.OneMarkMicros);
  sendDistanceWidthTiming.add(sTimingInfoToFB.OneSpaceMicros);
  sendDistanceWidthTiming.add(sTimingInfoToFB.ZeroMarkMicros);
  sendDistanceWidthTiming.add(sTimingInfoToFB.ZeroSpaceMicros);
...
void setup() {
  ...
  // Serial.println(sDecodedRawDataArray[0]);
  ...
  if (Firebase.ready()) {
    ...
    if (Firebase.RTDB.getString(&fbdo, "/IR-Signal-Choose/IRRawData")) {
      ...
      sDecodedRawDataArray[0] = strtoull(valueStr.c_str(), NULL, 10);
      Serial.print("Value: ");
      Serial.println(sDecodedRawDataArray[0]);
    } else {
    ...
    if (Firebase.RTDB.getInt(&fbdo, F("/IR-Signal-Choose/NumberOfBits"), &sNumberOfBits)) {
    ...
    if (Firebase.RTDB.getArray(&fbdo, "/IR-Signal-Choose/DistanceWidthTimingInfo")) {
      ...
      updateDistanceWidthTimingInfo(arrTimingInfoFB);
      printDistanceWidthTimingInfo();
    } else {
...
void loop() {
  ...
  if (tSendButtonIsActive) {
    ...
    if (IrReceiver.decodedIRData.protocol != UNKNOWN) {
      ...
      if (memcmp(&sTimingInfoToFB, &IrReceiver.decodedIRData.DistanceWidthTimingInfo, sizeof(sTimingInfoToFB)) != 0) {
        ...
        Serial.println();
        sTimingInfoToFB = IrReceiver.decodedIRData.DistanceWidthTimingInfo;  // copy content here
        // Serial.println(" ");
        // printDistanceWidthTimingInfo();
        // Serial.println(" ");
      } else {
      ...
      if (NumberOfBitsToFB != IrReceiver.decodedIRData.numberOfBits) {
        Serial.print(F("Store new numberOfBits="));
        NumberOfBitsToFB = IrReceiver.decodedIRData.numberOfBits;
        Serial.println(IrReceiver.decodedIRData.numberOfBits);
      }
      if (DecodedRawDataArrayToFB[0] != IrReceiver.decodedIRData.decodedRawDataArray[0]) {
        *DecodedRawDataArrayToFB = *IrReceiver.decodedIRData.decodedRawDataArray;  // copy content here
        Serial.print(F("Store new DecodedRawDataArrayToFB[0]=0x"));
        Serial.println(DecodedRawDataArrayToFB[0], HEX);
      }

      String rawDataString = String(DecodedRawDataArrayToFB[0], HEX);
      rawDataString.toUpperCase();
      String path = "/IR-Signal/0x" + rawDataString;
      ...
      if (Firebase.ready()) {
        Firebase.RTDB.setInt(&fbdo, pathIRRawData.c_str(), DecodedRawDataArrayToFB[0]);
        Firebase.RTDB.setInt(&fbdo, pathNumberOfBits.c_str(), NumberOfBitsToFB);
  ...
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 1000 || sendDataPrevMillis == 0)) {
    ...
    if (Firebase.RTDB.getString(&fbdo, "/IR-Signal-Choose/IRRawData")) {
      String valueStr = fbdo.stringData();
      unsigned long long intValue = strtoull(valueStr.c_str(), NULL, 10);
      if (sDecodedRawDataArray[0] != intValue) {
        sDecodedRawDataArray[0] = intValue;
